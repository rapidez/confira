<toggler>
    <div slot-scope="{ toggle, isOpen }" class="*:pt-2.5 first:*:pt-0 mb-2.5 flex w-full flex-col gap-2.5 divide-y divide-dashed border-b border-dashed pb-2.5">
        <div v-for="(item, index) in cart.items" v-show="isOpen || index < 2" class="flex gap-3">
            <img v-if="item.product.image" class="max-h-20 w-20 shrink-0 object-contain mix-blend-darken" :alt="item.product.name" :src="'/storage/{{ config('rapidez.store') }}/resizes/200/magento/catalog/product' + item.product.image.url.replace(config.media_url, '') + '.webp'">
